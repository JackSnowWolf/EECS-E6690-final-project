...
printALL(newNBclassifier_conventional)
```
...
NB_train_predict_conventional <- predict(NBclassfier_conventional, newdata = train)
NB_test_predict_conventional  <- predict(NBclassfier_conventional, newdata = test)
cfm_train_conventional <- confusionMatrix(NB_train_predict_conventional ,train$asLabel)
cfm_test_conventional  <- confusionMatrix(NB_test_predict_conventional,test$asLabel)
input.mat.train_conventional = as.matrix(cfm_train_conventional)
normalized.mat.train_conventional = sweep(input.mat.train_conventional, 2, colSums(input.mat.train_conventional), "/" )
melt.mat.train_conventional <- melt(normalized.mat.train_conventional)
ggplot(data = melt.mat.train_conventional, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + ggtitle("Train Dataset Confusion Matrix of NaiveBayesian conventional") +
  xlab("Prediction") + ylab("Actual") + labs(fill = "frequency")
cfm_train
```
## Confusion Matrix of Test dataset - Converntional Features
```{r}
...
ggplot(data = melt.mat.test_conventional, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + ggtitle("Test Dataset Confusion Matrix of NaiveBayesian Conventional Feature") +
  xlab("Prediction") + ylab("Actual") + labs(fill = "frequency")
...
printALL(newNBclassifier_social_media)
```
#### Confusion Matrix of Train dataset - Social Media
```{r}
library(dplyr)
library(tidyr)
library(ggplot2)
library(scales)
library(reshape2)
NB_train_predict_social_media <- predict(NBclassfier_social_media, newdata = train)
NB_test_predict_social_media  <- predict(NBclassfier_social_media, newdata = test)
cfm_train_social_media <- confusionMatrix(NB_train_predict_social_media,train$asLabel)
cfm_test_social_media  <- confusionMatrix(NB_test_predict_social_media,test$asLabel)
input.mat.train_social_media = as.matrix(cfm_train_social_media)
normalized.mat.train_social_media = sweep(input.mat.train_social_media, 2, colSums(input.mat.train), "/" )
melt.mat.train_social_media <- melt(normalized.mat.train_social_media)
ggplot(data = melt.mat.train_social_media, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + ggtitle("Train Dataset Confusion Matrix of NaiveBayesian social media feature") +
  xlab("Prediction") + ylab("Actual") + labs(fill = "frequency")
cfm_train_social_media
```
...
ggplot(data = melt.mat.test_social_media, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + ggtitle("Test Dataset Confusion Matrix of NaiveBayesian social media feature") +
  xlab("Prediction") + ylab("Actual") + labs(fill = "frequency")
